...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Name of the depth estimation model.",
    )
    parser.add_argument(
        "--recalc",
        action="store_true",
        default=False,
        help="Whether inference requires intrinsics on input",
    )
    parser.add_argument(
        "--device",
        type=str,
...
def infer_depth(model, args):
    ...
    for scene_name in tqdm(scene_names):
        ...
        for filename in filenames:
            ...
            # intrinsic_path = (
            #     dataset_root / scene_name / "intrinsics" / f"{filename}.json"
            # )
            ...
            #     K[0, :] *= 2048
            #     K[1, :] *= 1365
            out_dict = model.infer(img_path)
            depth, K, runtime = (
                out_dict["depth"],
                out_dict["inference_K"],
                out_dict["runtime"],
            )
            (dataset_root / scene_name / f"{model.name}").mkdir(
                parents=True, exist_ok=True
            )
            np.savez_compressed(
                dataset_root / scene_name / f"{model.name}" / f"{filename}",
        ...
        for model_name, weight_list in ALL_MDEs.items():
            for weights in weight_list:
                ...
                try:
                    ...
                    print(
                        f"Model {model_name} with weights {weights} failed with error: {e}"
                    )
                    traceback.print_exc()
