...
def encode_result(result):
    ...
    info = result['info']
    info_scalar_keys = sorted(k for k in info if k != 'inliers')
    buf.write(struct.pack('<I', len(info_scalar_keys)))
    for k in info_scalar_keys:
...
def save_full_results(args, full_results):
    ...
    with h5py.File(h5_path, 'w') as f:
        ...
        encoded = [r['encoded'] for r in full_results]
